...
class TestKnowledgeBaseBuildingProcess:
    ...
    def test_empty_kb_initialization_creates_structure(self, tmp_path):
        """
    ...
    @patch("src.build_kb.fitz")
    def test_build_kb_with_pdf_extraction_workflow(self, mock_fitz, mock_transformer, tmp_path):
        """
        Test complete KB build with PDF extraction.
        
        Given: Zotero data with PDF files
        When: build_kb is called
        Then: Extracts text, generates embeddings, creates index
        """
        # Mock PDF extraction
        mock_page = MagicMock()
        ...
        # Extract PDFs
        for paper in papers:
            paper["full_text"] = builder.extract_pdf_text(
                paper["pdf_path"],
                paper["zotero_key"],
                use_cache=False
            )
        ...
        # Verify results
        assert all(p.get("full_text") for p in papers)
        assert embeddings.shape == (2, 768)
        assert (tmp_path / "index.faiss").exists()

    def test_build_kb_idempotency_preserves_data(self, tmp_path):
        """
    ...
    def test_kb_integrity_after_build_validates_structure(self, tmp_path):
        """
    ...
    def test_pdf_extraction_coverage_reporting(self, tmp_path):
        """
        ...
        papers_with_text = sum(
            1 for p in papers 
            if p.get("full_text") and len(p["full_text"].strip()) > 0
        )
    ...
    def test_build_verification_output_format(self, tmp_path, capsys):
        """
        ...
        print(f"PDFs extracted: {stats['papers_with_pdfs']} ({stats['papers_with_pdfs']/stats['total_papers']*100:.1f}%)")
        
        if stats['papers_with_pdfs'] < stats['total_papers'] * 0.9:
            print(f"⚠️  WARNING: Low PDF coverage ({stats['papers_with_pdfs']/stats['total_papers']*100:.1f}% < 90%)")
...
class TestReportGeneration:
    ...
    def test_small_pdfs_report_identifies_issues(self, tmp_path):
        """
        ...
        small_papers = [
            p for p in papers
            if p.get("full_text") and len(p["full_text"]) < 5000
        ]
...
class TestCacheOperations:
    ...
    def test_cache_speeds_up_rebuilds(self, tmp_path):
        """
        ...
        cache_data = {
            "KEY0001": {
                "text": "Cached PDF content for paper 1",
                "hash": "abc123"
            },
            "KEY0002": {
                "text": "Cached PDF content for paper 2",
                "hash": "def456"
            }
        }
    ...
    def test_embedding_cache_incremental_updates(self, tmp_path):
        """
        Test that embedding cache supports incremental updates.
        
        Given: Existing embedding cache
        When: New papers are added
        Then: Only new papers get embedded
        """
        builder = KnowledgeBaseBuilder(knowledge_base_path=str(tmp_path))
        
        # Create initial embedding cache
        initial_embeddings = np.random.randn(5, 768).astype("float32")
        initial_hashes = ["hash1", "hash2", "hash3", "hash4", "hash5"]
        ...
        # Verify cache loaded
        assert cache["embeddings"].shape == (5, 768)
        assert len(cache["hashes"]) == 5
        
        # Add new embeddings
        new_embeddings = np.random.randn(2, 768).astype("float32")
        new_hashes = ["hash6", "hash7"]
        
        # Combine with existing
        combined_embeddings = np.vstack([cache["embeddings"], new_embeddings])
        combined_hashes = cache["hashes"] + new_hashes
        
        # Save updated cache
        builder.save_embedding_cache(combined_embeddings, combined_hashes)
        
        # Reload and verify
        updated_cache = builder.load_embedding_cache()
        assert updated_cache["embeddings"].shape == (7, 768)
        assert len(updated_cache["hashes"]) == 7
...
class TestCriticalKBIntegrity:
    ...
    def test_empty_kb_handling(self, tmp_path):
