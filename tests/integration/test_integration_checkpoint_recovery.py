...
class TestCheckpointRecovery:
    ...
    def test_checkpoint_cleanup_on_success(self):
        ...
        test_papers = [
            {"key": f"TEST_{i:04d}", "doi": f"10.1234/test{i}", "title": f"Test Paper {i}"} for i in range(5)
        ]

        with patch("requests.post") as mock_post:
            mock_response = Mock()
            mock_response.status_code = 200
            mock_response.json.return_value = [{"title": f"Paper {i}", "citationCount": i} for i in range(5)]
            mock_post.return_value = mock_response
    ...
    def test_checkpoint_preserves_error_results(self):
        ...
        with patch("requests.post") as mock_post:
            mock_response = Mock()
            mock_response.status_code = 200
            mock_response.json.return_value = [{"title": "Paper 3", "citationCount": 30}]
            mock_post.return_value = mock_response
    ...
    def test_corrupted_checkpoint_recovery(self):
        ...
        with patch("requests.post") as mock_post:
            mock_response = Mock()
            mock_response.status_code = 200
            mock_response.json.return_value = [{"title": "Paper 1", "citationCount": 10}]
            mock_post.return_value = mock_response
    ...
    def test_checkpoint_intervals(self, batch_size, total_papers):
        ...
        with patch("builtins.open", side_effect=track_checkpoint_open), patch("requests.post") as mock_post:
