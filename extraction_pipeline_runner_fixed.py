#!/usr/bin/env python3
"""Extraction Pipeline Runner - Fixed version with proper synchronization.
...
def verify_stage_completion(stage_dir, min_files=1):
    ...
    json_files = [f for f in stage_dir.glob("*.json") if "report" not in f.name]
...
def main():
    ...
    parser.add_argument(
        "--pipeline-dir",
        default=None,
        help="Pipeline directory (default: extraction_pipeline_fixed_YYYYMMDD)",
    )
    parser.add_argument(
        "--start-from",
    ...
    pipeline_stages = {
        ...
        "unpaywall": {
            "description": "Unpaywall OA discovery",
    ...
    for stage_dir in sorted(pipeline_dir.glob("*/")):
        json_count = len([f for f in stage_dir.glob("*.json") if "report" not in f.name])
