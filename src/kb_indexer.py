...
class KBIndexer:
    ...
    def generate_embeddings(self, texts: list[str], show_progress: bool = True) -> np.ndarray:
        ...
        if texts_to_embed:
            print(f"Generating embeddings for {len(texts_to_embed)} papers (reusing {len(texts) - len(texts_to_embed)} cached)...")
            ...
            for idx, embedding in zip(text_indices, new_embeddings_array):
    ...
    def update_index_incrementally(self, papers: list[dict[str, Any]], changes: dict[str, Any]) -> None:
        ...
        changed_keys = (changes.get("new_keys", set()) | 
                       set(changes.get("updated_keys", []))) - quality_upgrades
        ...
        if texts_to_embed:
            ...
            for i, embedding in zip(papers_to_embed, new_embeddings):
