...
class APIRateTester:
    ...
    async def test_single_request(self, doi: str = None, title: str = None) -> Dict[str, Any]:
    ...
    async def test_concurrent_requests(self, concurrent_count: int, total_requests: int) -> List[Dict[str, Any]]:
...
async def main():
    ...
    async with APIRateTester() as tester:
        ...
        # Test 1: Single requests to establish baseline
        ...
        print(f"  Baseline: {baseline_analysis['success_rate']:.1f}% success, {baseline_analysis['successful_response_time_stats']['mean']:.3f}s avg response")
        all_results.extend(baseline_results)
        
        # Test 2: Burst requests (no delay)
        ...
        print(f"  Burst: {burst_analysis['success_rate']:.1f}% success, {burst_analysis['rate_limit_percentage']:.1f}% rate limited")
        ...
        for concurrent in [1, 5, 10, 20]:
            ...
            print(f"  {concurrent} concurrent: {concurrent_analysis['success_rate']:.1f}% success, {concurrent_analysis['requests_per_second']:.2f} RPS")
            all_results.extend(concurrent_results)
            
            # Break if we're getting heavily rate limited
            if concurrent_analysis['rate_limit_percentage'] > 50:
                print(f"  ⚠️  High rate limiting detected, stopping concurrency tests")
                break
        ...
        # Performance implications for KB building
