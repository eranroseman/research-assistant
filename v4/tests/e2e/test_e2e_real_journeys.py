...
class TestRealUserJourneys:
    ...
    def run_cli_command(
        self, args: list[str], input_text: str | None = None, timeout: int = 30
...
class TestJourney1BasicResearchWorkflow(TestRealUserJourneys):
    ...
    def test_batch_command_workflow(self):
        ...
        batch_commands = [{"command": "info", "args": {}}, {"command": "diagnose", "args": {}}]
...
class TestJourney2AdvancedSearchWorkflow(TestRealUserJourneys):
    ...
    def test_export_functionality(self):
        ...
        result = self.run_cli_command(["search", "test", "--export", str(export_file), "--top-k", "5"])

        # Check if export option is available and export succeeded
        if "--export" not in result.stderr and result.returncode == 0 and export_file.exists():
            content = export_file.read_text()
            assert len(content) > 0


class TestJourney3CitationAndRetrieval(TestRealUserJourneys):
    ...
    def test_paper_quality_indicators(self):
        ...
        if "quality" in result.stdout.lower():
            ...
            assert (
                "A+" in result.stdout
                or "quality" in result.stdout.lower()
                or "score" in result.stdout.lower()
            )


class TestJourney4SystemDiagnostics(TestRealUserJourneys):
    ...
    def test_diagnose_command(self):
        ...
        assert result.returncode == 0
        assert (
            "diagnos" in result.stdout.lower()
...
class TestJourney5BuildAndUpdate(TestRealUserJourneys):
...
class TestJourney6ErrorHandling(TestRealUserJourneys):
